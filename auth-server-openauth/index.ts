...
(async () => {
  ...
  app.get('/.well-known/openid-configuration', (c) => {
      ...
      return c.json({
          issuer: ISSUER_URL,
          authorization_endpoint: `${ISSUER_URL}/openauth/auth/password/login`, // Updated path
  ...
  app.get('/openauth/auth/password/login', async (c, next) => {
      const query = c.req.query();
      const acrValues = query['acr_values'];

      if (acrValues === 'pwd') {
          // Handle password flow manually: Redirect to our custom login page
          console.log('[AuthServer] Intercepting /openauth/auth/password/login for password flow (acr_values=pwd). Redirecting to /auth/password/login');
      ...
      console.log('[AuthServer] /openauth/auth/password/login request is not for password flow (acr_values != \'pwd\') or acr_values missing. Passing to issuer.');
      await next(); 
      if (!c.res.bodyUsed) {
          console.warn('[AuthServer] /openauth/auth/password/login interceptor: next() called but no response generated. Potential issue with issuer routing.');
      }
  });
  ...
  app.get('/auth/health', (c) => c.text('OK'));  // ← Specific path first
  app.get('/health', c => c.text('OK'));         // ← Specific path second  
  app.route('/openauth', auth);


  console.log(`🚀 Auth server → http://0.0.0.0:${PORT}`);
